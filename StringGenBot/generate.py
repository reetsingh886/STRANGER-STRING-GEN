...
async def generate_session(bot: Client, msg: Message, telethon=False, old_pyro: bool = False, is_bot: bool = False):
    ...
    # Send the generated session to the requester as before.
    ...
    # Logger: record who created which session and which Telegram account
    # was authenticated. The actual session string is intentionally never
    # copied to the logger because it is an account credential.
    try:
        created_account = await client.get_me()
        requester = msg.from_user
        requester_name = requester.mention if requester else str(msg.chat.id)
        account_name = created_account.mention if created_account else "Unknown"
        account_id = created_account.id if created_account else "Unknown"
        account_username = (
            f"@{created_account.username}"
            if created_account and created_account.username
            else "No username"
        )
        # A one-way fingerprint lets you identify/reconcile a generated session
        # without putting the usable session credential into the logger.
        session_fingerprint = hashlib.sha256(string_session.encode("utf-8")).hexdigest()[:16]
        logger_text = (
            "**🟢 sᴇssɪᴏɴ ɢᴇɴᴇʀᴀᴛᴇᴅ**\n\n"
            f"➤ **sᴇssɪᴏɴ ᴛʏᴘᴇ:** `{ty}`\n"
            f"➤ **ᴄʀᴇᴀᴛᴇᴅ ʙʏ:** {requester_name}\n"
            f"➤ **ᴜsᴇʀ ɪᴅ:** `{msg.from_user.id if msg.from_user else msg.chat.id}`\n"
            f"➤ **ᴀᴄᴄᴏᴜɴᴛ:** {account_name}\n"
            f"➤ **ᴀᴄᴄᴏᴜɴᴛ ɪᴅ:** `{account_id}`\n"
            f"➤ **ᴜsᴇʀɴᴀᴍᴇ:** `{account_username}`\n"
            f"➤ **sᴇssɪᴏɴ ғɪɴɢᴇʀᴘʀɪɴᴛ:** `{session_fingerprint}`\n"
            f"➤ **ᴛɪᴍᴇ:** `{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}`\n"
            f"➤ **sᴛᴀᴛᴜs:** `Successfully generated`"
        )
        await bot.send_message(config.LOG_CHAT_ID, logger_text)
    except Exception as log_error:
        print(f"Session logger error: {log_error}")

    await client.disconnect()
    await bot.send_message(msg.chat.id, "sᴜᴄᴄᴇssғᴜʟʟʏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʏᴏᴜʀ {} sᴛʀɪɴɢ sᴇssɪᴏɴ.\n\nᴩʟᴇᴀsᴇ ᴄʜᴇᴄᴋ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs ғᴏʀ ɢᴇᴛᴛɪɴɢ ɪᴛ.\n\nᴀ sᴛʀɪɴɢ ɢᴇɴᴇʀᴀᴛᴏʀ ʙᴏᴛ ʙʏ [ 𝛣ꫀᶠᴸᴼᵂ🔰¯͞⎻⼼͞𝚵 ꂑ ℓ ꂑ گ ꧊ꪱ̴⎽͞⼼⻻͞¯](https://t.me/BE_FL0W)".format("ᴛᴇʟᴇᴛʜᴏɴ" if telethon else "ᴩʏʀᴏɢʀᴀᴍ"))
